...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, WordSearchGameComponent],
  template: `
    <div class="game-detail" *ngIf="game; else loading">
      ...
      <section class="game-panel">
        ...
        <app-word-search-game [examples]="game.examples || []"></app-word-search-game>
      </section>
    </div>
...
export class WordSearchPageComponent implements OnInit {
  game: JuegoDoc | null = null;
  private dataService = inject(DataService);

  ngOnInit(): void {
    this.dataService.getJuegosFromFirebase().subscribe((juegos) => {
      this.game = juegos.find((juego) => juego.id === 'word_search') ?? null;
    });
  }
}
