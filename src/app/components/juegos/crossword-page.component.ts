...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, CrosswordGameComponent],
  template: `
    <div class="game-detail" *ngIf="game; else loading">
      ...
      <section class="game-panel">
        ...
        <app-crossword-game [examples]="game.examples || []"></app-crossword-game>
      </section>
    </div>
...
export class CrosswordPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataService.getJuegosFromFirebase().subscribe((juegos) => {
      this.game = juegos.find((juego) => juego.id === 'crossword') ?? null;
    });
  }
}
