...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, GardenGameComponent],
  template: `
    <div class="game-detail" *ngIf="game; else loading">
      ...
      <section class="game-panel">
        ...
        <app-garden-game [examples]="game.examples || []"></app-garden-game>
      </section>
    </div>
...
export class GardenPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataService.getJuegosFromFirebase().subscribe((juegos) => {
      this.game = juegos.find((juego) => juego.id === 'garden') ?? null;
    });
  }
}
